...
void ofApp::update(){
    for (int i =0; i<100; i++){
        ...
        if (mouse_pressed){
            particle[i].speed_x =(mouseX -particle[i].loc_x) /8.0;
            particle[i].speed_y =(mouseY -particle[i].loc_y) /8.0;
        }
        
        particle[i].loc_x =particle[i].loc_x +particle[i].speed_x;
        particle[i].loc_y =particle[i].loc_y +particle[i].speed_y;
        
        if (particle[i].loc_x<0){
            particle[i].speed_x =particle[i].speed_x * -1;
        }
        
        if (particle[i].loc_x>ofGetWidth()){
            particle[i].speed_x =particle[i].speed_x * -1;
        }
        
        if (particle[i].loc_y<0){
            particle[i].speed_y =particle[i].speed_y * -1;
        }
        
        if (particle[i].loc_y>ofGetHeight()){
            particle[i].speed_y =particle[i].speed_y * -1;
        }
    }
}
